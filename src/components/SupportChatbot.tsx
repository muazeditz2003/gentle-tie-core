    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-[5.75rem] right-4 md:bottom-6 md:right-6 z-50 h-12 rounded-2xl border border-border/60 bg-gradient-brand px-4 text-primary-foreground shadow-premium hover:opacity-95 flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] group"
          aria-label="Open support chat"
        >
          <Bot className="h-5 w-5" />
          <span className="text-xs font-semibold">AI Assistant</span>
          <span className="absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-success border-2 border-background animate-pulse" />
        </button>
      )}

      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-24 left-4 right-4 md:left-auto md:bottom-6 md:right-6 z-50 md:w-[380px] max-w-[calc(100vw-2rem)] h-[70vh] md:h-[520px] max-h-[calc(100vh-7rem)] md:max-h-[calc(100vh-4rem)] bg-card border border-border/60 rounded-2xl shadow-premium flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
