-- Ensure one profile and one worker record per auth user
CREATE UNIQUE INDEX IF NOT EXISTS profiles_user_id_unique_idx ON public.profiles (user_id);
CREATE UNIQUE INDEX IF NOT EXISTS workers_user_id_unique_idx ON public.workers (user_id);

-- Make signup provisioning idempotent and resilient to malformed metadata
...
BEGIN
  ...
  INSERT INTO public.profiles (user_id, full_name, phone, city, avatar_url)
  VALUES (
    NEW.id,
